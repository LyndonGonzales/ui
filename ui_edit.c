...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	// Create the main text edit control for conversation history
	hMainEdit = CreateWindowEx(
    WS_EX_CLIENTEDGE, "EDIT", "", WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_AUTOVSCROLL | ES_READONLY,
    0, 0, 0, 0, hwnd, (HMENU)ID_EDIT, hInstance, NULL
	);
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
		case WM_SIZE: {
			// Resize the main edit control, text edit control, and send button to fit the window
			int width = LOWORD(lParam);
			int height = HIWORD(lParam);
			MoveWindow(hMainEdit, 10, 10, width - 20, height - 90, TRUE);
			MoveWindow(hEdit, 10, height - 70, width - 100, 60, TRUE);
			MoveWindow(hButton, width - 80, height - 70, 70, 30, TRUE);
			break;
		}
        case WM_COMMAND: {
        ...
        // Append the text to the main edit control
        int length = GetWindowTextLength(hMainEdit);
        SendMessage(hMainEdit, EM_SETSEL, (WPARAM)length, (LPARAM)length);
        SendMessage(hMainEdit, EM_REPLACESEL, FALSE, (LPARAM)buffer);
        SendMessage(hMainEdit, EM_REPLACESEL, FALSE, (LPARAM)"\r\n");

        // TODO: Send the text to OpenAI for chat completion
        // Simulate a response for demonstration purposes
        const char* response = "OpenAI: This is a simulated response.\r\n";
        length = GetWindowTextLength(hMainEdit);
        SendMessage(hMainEdit, EM_SETSEL, (WPARAM)length, (LPARAM)length);
        SendMessage(hMainEdit, EM_REPLACESEL, FALSE, (LPARAM)response);
